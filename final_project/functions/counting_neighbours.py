...
def counting_neighbour_values_of_each_cell(matrix, i, j, boundary_type):
    ...
    if boundary_type=="non_periodic":
        ...
        """ 
        In the non-periodic scenario, this function accounts for the edge 
        cells by only considering the neighboring cells that are within the 
        boundaries of the matrix. Essentially, it's akin to applying zero 
        padding around the matrix. This means that the edge cells will treat 
        any position outside the matrix as having a value of zero when calculating 
        neighboring cells.
        """
    ...
    if boundary_type=="periodic":
        ...
        Torus topology. This means that the matrix wraps around both horizontally 
        and vertically, allowing the edges to connect seamlessly which means
        the cell values do not change under the following transformations:
